 ...
 */
export class SwaggerReduxAnalyzer {
      ...
      for (const method of methods) {
        ...
        if (operation && operation.tags && operation.tags.includes(tagName)) {
          operations.push({
            ...
            responseSchema: this.getResponseSchemaNameForOperation(operation)
          });
        }
      }
    }
  ...
  public getResponseTypeSummary(path: string, method: string): string {
    const pathItem = this.openApiDoc?.paths?.[path];
    if (!pathItem || typeof pathItem !== 'object') return 'unknown';
   ...
   */
  public getResponseSchema(path: string, method: string): ResponseSchema | null {
    const pathItem = this.openApiDoc?.paths?.[path];
   ...
   */
  private parseResponseSchema(schema: any): ResponseSchema {
    let isArray = false;
    ...
    if (schema.$ref) {
      const refName = schema.$ref.split('/').pop();
      schemaName = refName || undefined;
      
      if (this.openApiDoc?.components?.schemas && refName) {
        const refSchema = this.openApiDoc.components.schemas[refName] as any;
        if (refSchema) {
          return this.parseResponseSchema(refSchema);
        }
      }
    }

    // 🆕 DETECCIÓN AUTOMÁTICA DEL CAMPO "response"
    // Si el schema tiene un campo "response", extraerlo automáticamente
    if (schema.properties && schema.properties.response) {
      console.log('🔍 Detectado campo "response" en el schema, extrayendo automáticamente...');
      const responseField = schema.properties.response;
      
      // Si tiene anyOf, buscar el schema válido
      if (responseField.anyOf) {
        for (const subSchema of responseField.anyOf) {
          if (subSchema.$ref) {
            ...
            if (this.openApiDoc?.components?.schemas && refName) {
              const refSchema = this.openApiDoc.components.schemas[refName] as any;
              if (refSchema) {
                console.log(`✅ Usando schema del campo "response": ${refName}`);
                return this.parseResponseSchema(refSchema);
              }
            }
          } else if (subSchema.type === 'array') {
            // Es un array directamente
            console.log('✅ Usando array del campo "response"');
            return this.parseResponseSchema(subSchema);
          } else if (subSchema.properties) {
            // Es un objeto con propiedades
            console.log('✅ Usando objeto del campo "response"');
            return this.parseResponseSchema(subSchema);
          }
        }
      } 
      // Si tiene $ref directamente
      else if (responseField.$ref) {
        const refName = responseField.$ref.split('/').pop();
        if (this.openApiDoc?.components?.schemas && refName) {
          const refSchema = this.openApiDoc.components.schemas[refName] as any;
          if (refSchema) {
            console.log(`✅ Usando schema del campo "response": ${refName}`);
      ...
      // Si es un array o objeto directo
      else if (responseField.type === 'array' || responseField.properties) {
        console.log('✅ Usando schema directo del campo "response"');
        return this.parseResponseSchema(responseField);
      }
    }
    ...
    if (schema.type === 'array' && schema.items) {
      ...
      if (schema.items.$ref) {
        ...
        if (this.openApiDoc?.components?.schemas && itemRefName) {
          const itemSchema = this.openApiDoc.components.schemas[itemRefName] as any;
          if (itemSchema && itemSchema.properties) {
            fields = this.extractFieldsFromSchema(itemSchema);
            schemaName = itemRefName;
          }
        }
      } else if (schema.items.properties) {
        fields = this.extractFieldsFromSchema(schema.items);
        schemaName = 'InlineItem';
      }
    }
    // Manejar objetos
    else if (schema.properties) {
      fields = this.extractFieldsFromSchema(schema);
      schemaName = schemaName || 'InlineResponse';
    }
    // Manejar anyOf
    else if (schema.anyOf) {
      for (const subSchema of schema.anyOf) {
        if (subSchema.type !== 'null') {
          const result = this.parseResponseSchema(subSchema);
   ...
   */
  private extractFieldsFromSchema(schema: any): EntityField[] {
    const fields: EntityField[] = [];
  ...
  public async loadFromFile(filePath: string): Promise<void> {
    try {
      ...
      // SwaggerParser.validate espera la RUTA del archivo, no el contenido
      this.openApiDoc = await SwaggerParser.validate(filePath) as OpenAPIV3.Document;
      this.detectedApiName = this.detectApiName();
      
      console.log(`✅ Archivo cargado exitosamente`);
    } catch (error) {
      throw new Error(`Error cargando archivo: ${error}`);
    }
  }
   ...
   */
  public getAvailableSchemasFromFile(): string[] {
    if (!this.openApiDoc?.components?.schemas) {
      return [];
    }

    return Object.keys(this.openApiDoc.components.schemas).sort();
  }
   ...
   * @returns ResponseSchema con estructura analizada
   */
  public getSchemaByName(schemaName: string): ResponseSchema | null {
    if (!this.openApiDoc?.components?.schemas) {
      return null;
    }

    const schema = this.openApiDoc.components.schemas[schemaName];
    if (!schema) return null;
